...
def calculate_spr(dataset, model_pred_data_path, pred):
    ...
    for i in range(num):
        ...
        # SA-SOR
        pred_masks = pred[i]['predictions_for_ranking']
        sa_sor, rank_vis = evalu(instance_masks, gt_ranks, pred_masks, 0.1)
        if sa_sor != -2:
            sasor_list.append(sa_sor)
            pred_labels = np.append(pred_labels, rank_vis)
            true_label = [int(rank*10-5) for rank in gt_ranks]
            l = len(rank_vis) - len(gt_ranks)
            if l > 0:
                true_label = np.append(true_label, [0 for i in range(l)])
    ...
    mapping = {1:5, 2:4, 3:3, 4:2, 5:1, 0:0}
    true_labels = np.array([mapping[label] for label in true_labels])
    pred_labels = np.array([mapping[label] for label in pred_labels])
    ...
    ax.set(xticks=np.arange(cm.shape[1]),
            ...
            xticklabels=['bg', '5', '4', '3', '2', '1'],
            yticklabels=['bg', '5', '4', '3', '2', '1'],
            ylabel='True label',
    ...
    for i in range(cm.shape[0]):
        for j in range(cm.shape[1]):
            ax.text(j, i, cm[i, j], ha="center", va="center", color="white" if cm[i, j] > cm.max() / 2. else "black",
                    fontsize=16)
    fig.tight_layout()
    fig.savefig(model_pred_data_path.replace('saliency_map/', 'Confusion-Matrix of PSR'), dpi=500)

    return spr_data, sasor_list
...
def eval_sor(map_path, pred, dataset_root, data_split='test'):
    ...
    ####################################################
    # Calculate SOR & SA
    spr_data, sa_sor = calculate_spr(dataset, map_path, pred)
    image_used, sor = eval_spr(spr_data)
    sa_sor = sum(sa_sor)/len(sa_sor)
    print('sa_sor:', sa_sor)
    ####################################################
    return {
        'mae': avg_mae,
...
if __name__ == '__main__':
    ...
    DATASET_ROOT = "/path/"   # Change to your location
    data_split = "test"
    dataset = DatasetTest(DATASET_ROOT,
                          data_split, eval_spr=True)
